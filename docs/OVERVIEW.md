...
`dashboard`, `profile`, `reference-data` and `settings`, which pull in
...
`/dashboard` and `/settings` are called here and served by nothing.
`/dashboard` is the near miss: assessment-service serves `/dashboards`,
plural, a teacher's or student's own slice, a different shape. `/settings`'s
`AppSetting` was an admin screen that did not survive the split;
`admin-web` is where it should land. Both sit in `UNROUTED_PREFIXES`, so they
404 in dev exactly as the gateway 404s them.
...
The gateway serves it at `hr.localhost`. `infra/nginx/*.conf` are generated
